...
(defn sort-table-by-column [table column]
  (let [rows (js->clj (.getElementsByTagName js/document table "tr") :keywordize-keys true)
        get-cell-text (fn [row]
                        (-> row
                            (.getElementsByTagName js/document "td")
                            (aget column)
                            (.textContent js/document)
                            (.trim)))]
...
(def table (.getElementsById js/document "myTable"))
              ...
              (e/client
                (let [!state (atom {
                  ...
                  (dom/div (dom/props {:style {:position "relative" :top "50%" :left "5%"}})
                           (dom/button
                             (dom/on "click" (fn [] ("sort-table" (.getElementById js/document "myTable") 1)))
                             (dom/text "Sort")
                           ...
                           (dom/table
                             ...
                             (dom/tr
                               (dom/td (dom/text "1"))
                               (dom/td (dom/text "John"))
                               (dom/td (dom/text "25"))
                               )
                             (dom/tr
                               (dom/td (dom/text "2"))
                               (dom/td (dom/text "Alice"))
                               (dom/td (dom/text "30"))
                               )
                  ...
                  (dom/p (dom/text "asd "  (js->clj (.getElementsByTagName js/document "tr") :keywordize-keys true)))
                  (dom/p (dom/text "asd "  (sort-table-by-column table 0)))
                  (dom/p (dom/text "asd "  table))
